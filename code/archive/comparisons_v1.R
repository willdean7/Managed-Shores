...
redfin_sf <- redfin_sf |>
  # ensure ADDRESS field is normalized for the join
  dplyr::mutate(ADDRESS = trimws(tolower(ADDRESS))) |>
  # join on address
  dplyr::left_join(runup_lookup, by = c("ADDRESS" = "address")) |>
  # force both parcel_id versions to character before merging
  dplyr::mutate(
    parcel_id.x = as.character(parcel_id.x),
    parcel_id.y = as.character(parcel_id.y),
    parcel_id   = dplyr::coalesce(parcel_id.x, parcel_id.y)
  ) |>
  # clean up leftover columns
  dplyr::select(-dplyr::matches("parcel_id\\.x|parcel_id\\.y"))
