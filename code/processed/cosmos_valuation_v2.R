...
if (RUN_SENSITIVITY) {
  ...
  message("\n========================================")
  message("RUNNING FULL FACTORIAL SENSITIVITY ANALYSIS")
  message("Generating ALL parameter combinations...")
  message("========================================")
  
  # Create full factorial design using expand.grid
  # This generates EVERY combination of parameters
  ...
  param_grid <- expand.grid(
    ...
    storm_scenario = SENSITIVITY_PARAMS$storm_scenario,
    stringsAsFactors = FALSE
  )
  
  # Remove the baseline row (we already ran it)
  ...
  message("\nTotal parameter combinations: ", n_scenarios)
  message("  (This will take approximately ", round(n_scenarios / 10), "-", round(n_scenarios / 5), " minutes)")
  message("  With 113 properties × 3 SLR scenarios × ", n_scenarios, " combos = ", 
          format(113 * 3 * n_scenarios, big.mark = ","), " calculations\n")
  
  # Create descriptive labels for each combination
  param_grid <- param_grid %>%
    mutate(
      label = sprintf(
        "y%.0f_d%.1f_t%.0f_u%.0f_%s",
        rental_yield * 100,      # y5 = 5% yield
        discount_rate * 100,     # d2.0 = 2% discount
        damage_threshold * 100,  # t15 = 15cm threshold
        owner_utility / 1000,    # u0 = $0K utility
        storm_scenario           # w000 or w100
      )
    )
  ...
  for (i in 1:nrow(param_grid)) {
    ...
    if (i %% 10 == 0 || i == 1) {
      ...
      message(sprintf("  Progress: %d/%d (%.1f%%) - Est. %.0f min remaining",
                      i, n_scenarios, 100 * i / n_scenarios, remaining))
    }
    ...
    result <- run_scenario(
      ...
      storm_scenario = params$storm_scenario,
  ...
  message("\n✓ Completed ", n_scenarios, " scenarios in ", round(total_time, 1), " minutes")
